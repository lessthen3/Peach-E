...
int main(int fp_ArgCount, char* fp_ArgVector[])
{
    ...
    // Setup Nuklear GUI
    ////////////////////////////////////////////////
    struct nk_context* ctx = nk_sdl_init(editor_renderer->GetMainWindow());

    struct nk_font_atlas* mf_FontAtlas;
    nk_sdl_font_stash_begin(&mf_FontAtlas);

    struct nk_font* mf_ComicSans = nk_font_atlas_add_from_file(mf_FontAtlas, "../fonts/Comic Sans MS", 14, 0);

    nk_sdl_font_stash_end();

    //nk_init_fixed(&ctx, calloc(1, MAX_MEMORY), MAX_MEMORY, mf_ComicSans);
