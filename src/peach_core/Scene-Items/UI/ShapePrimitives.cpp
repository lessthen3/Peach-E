        ...
        return std::visit([&fp_Origin, &fp_TestPoint](const auto& fv_S) noexcept -> bool
            {
                ...
                else if constexpr (std::is_same_v<T, CircleShape>)
                {
                    ...
                    return glm::length2(glms_vec2_sub(fp_TestPoint, fp_Origin)) <= (fv_S.Radius * fv_S.Radius);
                }
                ...
                else if constexpr (std::is_same_v<T, PolygonShape>)
                {
                    ...
                    {
                        vec2s fv_Curr = fp_Origin + f_Verts[lv_I];
                        vec2s fv_Next = fp_Origin + f_Verts[(lv_I + 1) % f_N];

                        vec2s fv_Edge = fv_Next - fv_Curr;
                        vec2s fv_ToPoint = fp_TestPoint - fv_Curr;
        ...
        return std::visit([&fp_Origin](const auto& fv_S) noexcept -> glm::vec4
            {
                ...
                {
                    vec2s f_C1 = fp_Origin + fv_S.LocalCenter1;
                    vec2s f_C2 = fp_Origin + fv_S.LocalCenter2;
