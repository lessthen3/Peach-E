...
namespace PeachCore {
    ...
    Texture2D::Texture2D(const std::string& name, const std::string& imagePath) : m_Name(name) 
    {
    ...
    void Texture2D::DeleteTexture() //resets the texture2D object
    {
        if (pm_IsValid) {
    ...
    std::vector<std::tuple<float, float, float, float>> Texture2D::GetTileUVs()
        const
    {
        return pm_TileUVs;
    }
    std::tuple<float, float, float, float> Texture2D::GetTileUV(const int tileIndex) 
        const 
    {
    ...
    const sf::Texture& Texture2D::GetSFMLTexture() 
        const 
    {
        return pm_Texture;
    }
    int Texture2D::GetTileCount() 
        const 
    {
