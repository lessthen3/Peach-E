...
namespace PeachCore {
    ...
    struct TextureData
    {
        ...
        unique_ptr<unsigned char, decltype(&stbi_image_free)> m_TextureByteData;
        ...
        TextureData(unsigned char* ptr, const int w, const int h, const int c)
            : m_TextureByteData(ptr, stbi_image_free), m_TextureWidth(w), m_TextureHeight(h), m_NumColourChannels(c) {}

        TextureData() : m_TextureByteData(nullptr, nullptr), m_TextureWidth(0), m_TextureHeight(0), m_NumColourChannels(0) {}
    };
    ...
    struct LoadedResourcePackage
    {
        ...
        LoadedResourcePackage(const string& fp_ID, TextureData& fp_TextureData)
            : PeachObjectID(move(fp_ID)), ResourceData(move(fp_TextureData)) {}

        LoadedResourcePackage(const string& fp_ID, AudioData& fp_AudioData)
            : PeachObjectID(move(fp_ID)), ResourceData(move(fp_AudioData)) {}

        LoadedResourcePackage(const string& fp_ID, unique_ptr<nlohmann::json> fp_JSONData)
            : PeachObjectID(move(fp_ID)), ResourceData(move(fp_JSONData)) {}
