...
namespace PeachCore {
		...
		)
	{
		...
		resource_logger->Initialize("resource_thread", fp_LogOutputDirectory, "ResourceLoadingManager", fp_Console);
		...
		) //This method should be one of the first methods called on startup
	{
		...
		{
			resource_logger->LogAndPrint("Attempted to get more than 2 references to PeachEngineResourceLoadingManager's AudioResourceLoadingQueue", "ResourceLoadingManager", LogManager::LogLevel::Warning);
			return nullptr;
		}
		...
		) //This method should be one of the first methods called on startup
	{
		...
		{
			resource_logger->LogAndPrint("Attempted to get more than 2 references to PeachEngineResourceLoadingManager's DrawableResourceLoadingQueue", "ResourceLoadingManager", LogManager::LogLevel::Warning);
			return nullptr;
		}
	...
	bool ResourceLoadingManager::TryPushingLoadedResourcePackage(unique_ptr<LoadedResourcePackage> fp_LoadedPackage)
	{
		...
		{
			resource_logger->LogAndPrint("Load put off until later", "ResourceLoadingManager", LogManager::LogLevel::Trace);
			return false;
		}
